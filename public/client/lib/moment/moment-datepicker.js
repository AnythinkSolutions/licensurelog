...
!function ($, moment, undefined) {
    ...
    Datepicker.prototype = {
        ...
        setStartDate: function(newStartDate){
            this.startDate = DPGlobal.parseDate(newStartDate, this.format);
        },

        setEndDate: function(newEndDate){
            this.endDate = DPGlobal.parseDate(newEndDate, this.format);
//            if(moment(newEndDate) < this.get())
        },

        place: function () {
            var sourceItem = this.component ? this.component : this.element;
            var offset = sourceItem.offset();
            var offsetTop = sourceItem[0].offsetTop;

            if (this.calendarPlacement == 'left') {
                this.picker.css({
                    top: offset.top + this.height, //offset.top + this.height,
