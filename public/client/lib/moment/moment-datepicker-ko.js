(function ($, ko, moment) {
    ...
    ko.bindingHandlers.datepicker = {
        init: function (element, valueAccessor, allBindingsAccessor) {
            ...
            if(options.startDate && ko.isObservable(options.startDate)){
                options.startDate.subscribe(function(newVal){
                   $el.datepicker('setStartDate', newVal);
                });

                options.startDate = ko.utils.unwrapObservable(options.startDate);
            }
            if(options.endDate && ko.isObservable(options.endDate)){
                options.endDate.subscribe(function(newVal){
                    $el.datepicker('setEndDate', newVal);
                });

                options.endDate = ko.utils.unwrapObservable(options.endDate);
            }
